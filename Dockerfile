...
RUN apk update \
  ...
  && curl -s https://api.github.com/repos/jkeys089/lua-resty-hmac/releases/latest | grep tarball_url | cut -d '"' -f 4 | wget -qi- --output-document=lua-resty-hmac.gz.tar \
...
RUN tar -zxf /tmp/lua-resty-http.gz.tar \
  ...
  && tar -zxf /tmp/lua-resty-hmac.gz.tar \
  ...
  && cp -r jkeys089-lua-resty-hmac*/lib/resty/* /tmp/openresty/ \
